...
<div class="flex justify-end gap-3 pt-5 px-2" v-if="user">
    ...
    >
        ...
        <span class="hidden sm:inline text-gray-700 group-hover:text-green-600 font-medium">My orders / 我的訂單</span>
        ...
        <div class="py-4 sm:py-6">
            <div class="mx-4 mb-6 text-sm sm:text-base text-gray-600 space-y-2">
                <p>歡迎參與"畢業生限量專屬紀念品"預購活動！本校特意為準畢業生製作了3款專屬紀念品，濃縮校園記憶，注入點滴情誼，期望澳理大的關懷能陪伴畢業生走過更多的人生角色與秋冬。本次預購活動純屬自願性質，收益將全數作為大學收入用於母校發展。有興趣的同學可填寫下表，並繳付預購金額，支持本次活動。在此，祝願同學順利畢業，前程錦繡！</p>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-3 sm:p-4">
                <div v-for="product in products" :key="product.id" 
                    ...
                    <div class="w-full sm:w-64 h-48 sm:h-40 flex-shrink-0 bg-gray-200">
                        <a-carousel :autoplay="true" dots class="h-full">
                            <div v-for="(image, index) in product.images" :key="index" class="h-48 sm:h-40">
                    ...
                    <div class="flex-grow p-4 sm:p-4">
                        ...
                        <a-button type="primary" 
                                 ...
                                 :disabled="user == null || selectedProductCount(product.id) >= product.quota"
                                 :class="[
            ...
            <div v-else class="flex flex-col h-full">
                <div class="flex-1 overflow-y-auto">
                        ...
                        <li v-for="item in cartItems" :key="item.id" 
                            ...
                            <div class="flex items-center justify-between sm:justify-end gap-4">
                                <div class="flex items-center space-x-2 bg-white rounded-lg border">
                                    <a-button @click="decreaseCount(item)" 
                                             size="small" 
                                             type="text"
                                    ...
                                    <a-button @click="increaseCount(item)" 
                                             size="small" 
                                             type="text"
                                             class="!px-3 hover:bg-gray-100">
                                        +
                    ...
                    <a-form :model="orderForm" 
                        ...
                        <a-form-item label="Faculty / 系所" name="faculty" class="mb-0">
                            <a-select v-model:value="orderForm.faculty" 
                                     placeholder="Select your faculty / 選擇您的系所"
                                     class="w-full">
                                <a-select-option v-for="faculty in faculties" 
                                                :key="faculty.value" 
                                                :value="faculty.value">
                                    {{ faculty.label }}
                                </a-select-option>
                        ...
                        <a-form-item label="Degree / 學位" name="degree" class="mb-0">
                            <a-select v-model:value="orderForm.degree" 
                                     placeholder="Select your degree / 選擇您的學位"
                                     class="w-full">
                        ...
                        <a-form-item label="Contact Phone Number / 聯絡電話" name="phone" class="mb-0">
                            <a-input v-model:value="orderForm.phone" 
                                    placeholder="Enter your phone number / 輸入您的電話號碼"
                                    class="w-full" />
                        </a-form-item>

                        <a-form-item label="Personal Email (optional) / 個人電子郵件（可選）" name="email" class="mb-0">
                            <a-input v-model:value="orderForm.email" 
                                    placeholder="Enter your email / 輸入您的電子郵件"
                                    class="w-full" />
                        </a-form-item>
                    </a-form>
                </div>
                ...
                <div class="border-t pt-4 mt-4 space-y-3">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a-button @click="selectedIsOpen = false" 
                                 class="flex-1 h-10">
                            Close / 關閉
                        </a-button>
                    </div>
        ...
        <a-drawer title="Orders / 訂單" 
                  :visible="orderIsOpen" 
                  @close="orderIsOpen = false"
                  placement="right"
                  :width="windowWidth <= 640 ? '100%' : '400px'">
                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    <template v-else>
                        <div class="mb-4 text-right">
                            <a-button :href="route('souvenir.pickupCode')" 
                                     danger 
                                     ghost 
                                     size="small">
                                取件碼
                        ...
                        <div v-for="(order, index) in user.orders" 
                             :key="index" 
